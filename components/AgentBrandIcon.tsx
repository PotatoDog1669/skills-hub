...
const BRAND_IMAGE_SOURCES: Partial<Record<AgentBrand, string>> = {
...
export function inferAgentBrand(agentName: string): AgentBrand | null {
  ...
  if (normalized.includes('kimi')) return 'kimi-code-cli'
...
export function AgentBrandIcon({
  ...
  const imageSource = BRAND_IMAGE_SOURCES[app]
  ...
  if (imageSource) {
    return (
      <img
        ...
        aria-hidden="true"
        data-testid={`agent-brand-${app}`}
      />
    )
  }

  if (app === 'github-copilot') {
    return <Github className={className} aria-hidden="true" data-testid={`agent-brand-${app}`} />
  }
  ...
  if (app === 'claude') {
    return (
      <svg
        ...
        data-testid={`agent-brand-${app}`}
      >
        <path d={CLAUDE_ICON_PATH} fill="#D97757" />
      </svg>
    )
  }
  ...
  if (app === 'codex') {
    return (
      <svg
        ...
        data-testid={`agent-brand-${app}`}
      >
        <path d={CODEX_ICON_PATH} fillRule="evenodd" fill="#111827" />
      </svg>
    )
  }
  ...
  if (app === 'cursor') {
    return (
      <svg
        ...
        data-testid={`agent-brand-${app}`}
      >
        <path d={CURSOR_ICON_PATH} fillRule="evenodd" fill="currentColor" />
      </svg>
    )
  }
  ...
  return (
    ...
    >
      ...
      <path d={GEMINI_ICON_PATH} fill={`url(#${fill0})`} />
      <path d={GEMINI_ICON_PATH} fill={`url(#${fill1})`} />
      <path d={GEMINI_ICON_PATH} fill={`url(#${fill2})`} />
    </svg>
  )
}
