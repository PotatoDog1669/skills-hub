...
import { X, Trash2, Plus, RefreshCw, Terminal, CheckCircle2, AlertCircle } from 'lucide-react';
        ...
        <div className={styles.overlay} onClick={onClose}>
            <div className={styles.modal} onClick={e => e.stopPropagation()}>
                ...
                <div className={styles.content}>
                    <div className={styles.section}>
                        ...
                        <div className={styles.list}>
                            ...
                            {(!config.scanRoots || config.scanRoots.length === 0) && (
                                <div className="text-sm text-muted-foreground italic p-2 text-center">No workspaces configured</div>
                            )}
                        </div>
                    </div>
                    ...
                    {/* Status Message */}
                    {message && (
                        <div className={`text-sm p-3 rounded flex items-center gap-2 ${message.type === 'success' ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'}`}>
                            {message.type === 'success' ? <CheckCircle2 size={16} style={{ transform: 'translateY(1.5px)' }} /> : <AlertCircle size={16} style={{ transform: 'translateY(1.5px)' }} />}
                            {message.text}
                        </div>
                    )}
                </div>
