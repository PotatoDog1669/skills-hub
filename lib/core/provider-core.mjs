...
function normalizeCodexAuthConfig(authConfig) {
  ...
  if (typeof normalized.api_key !== 'string' && typeof normalized.OPENAI_API_KEY === 'string') {
    normalized.api_key = normalized.OPENAI_API_KEY
  }
...
function mergeLiveConfig(appType, liveConfig, providerConfig) {
  ...
  if (appType === 'codex') {
    const liveAuth = normalizeCodexAuthConfig(liveConfig?.auth || {})
    const providerAuth = normalizeCodexAuthConfig(sanitizedProviderConfig?.auth || {})
    return {
      auth: deepMergeObject(liveAuth, providerAuth),
      configToml: deepMergeObject(
        liveConfig?.configToml || {},
        sanitizedProviderConfig?.configToml || {}
      ),
    }
  }
...
async function captureProviderFromLive({ appType, name, profile }) {
  ...
  const provider = addProvider({
    appType,
    name,
    config: attachProfile(liveConfig, {
      ...profile,
      kind: 'official',
    }),
  })
