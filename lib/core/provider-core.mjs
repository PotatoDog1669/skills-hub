...
function parseProviderRow(row) {
  ...
  if (provider.appType === 'codex') {
    provider.config = preserveProviderProfile(
      readCodexProviderSnapshot(provider.id, provider.config),
      provider.config
    )
  }
...
function getCodexProviderSnapshotDir(providerId) {
  return path.join(DB_DIR, 'provider-snapshots', 'codex', providerId)
}

function getCodexProviderAuthSnapshotPath(providerId) {
  return path.join(getCodexProviderSnapshotDir(providerId), 'auth.json')
}

function getCodexProviderConfigSnapshotPath(providerId) {
  return path.join(getCodexProviderSnapshotDir(providerId), 'config.toml')
}

function getLegacyCodexProviderAuthSnapshotPath(providerId) {
  return path.join(DB_DIR, 'provider-auth', 'codex', providerId, 'auth.json')
}

function readJsonFileSyncOrNull(filePath) {
  try {
...
function readTextFileSyncIfExists(filePath) {
  try {
...
function writeCodexProviderSnapshot(providerId, providerConfig) {
  ...
  if (normalized.auth && typeof normalized.auth === 'object' && !Array.isArray(normalized.auth)) {
    writeJsonAtomicSync(getCodexProviderAuthSnapshotPath(providerId), normalized.auth)
  }

  if (typeof normalized.config === 'string') {
    atomicWriteFileSync(getCodexProviderConfigSnapshotPath(providerId), normalized.config)
  }
}

function readCodexProviderSnapshot(providerId, fallbackConfig = null) {
  if (!providerId) return normalizeCodexProviderConfig(fallbackConfig)

  const auth =
    readJsonFileSyncOrNull(getCodexProviderAuthSnapshotPath(providerId)) ||
    readJsonFileSyncOrNull(getLegacyCodexProviderAuthSnapshotPath(providerId))
  const configPath = getCodexProviderConfigSnapshotPath(providerId)
  const hasConfigSnapshot = fs.existsSync(configPath)
  const config = hasConfigSnapshot ? readTextFileSyncIfExists(configPath) : null
  const fallback = normalizeCodexProviderConfig(fallbackConfig)

  return {
    ...(auth || fallback.auth ? { auth: auth || fallback.auth } : {}),
    ...(hasConfigSnapshot || typeof fallback.config === 'string'
      ? { config: hasConfigSnapshot ? config || '' : fallback.config }
      : {}),
  }
}

function removeCodexProviderSnapshot(providerId) {
  if (!providerId) return

  const snapshotDir = getCodexProviderSnapshotDir(providerId)
  if (fs.existsSync(snapshotDir)) {
    fs.rmSync(snapshotDir, { recursive: true, force: true })
  }

  const legacyDir = path.dirname(getLegacyCodexProviderAuthSnapshotPath(providerId))
...
function addProvider({ appType, name, config }) {
  ...
  if (
    ...
    writeCodexProviderSnapshot(id, normalizedConfig)
  }
...
function updateProvider({ id, name, config }) {
  ...
  if (
    ...
    writeCodexProviderSnapshot(existing.id, nextConfig)
  }
...
function deleteProvider(id) {
  ...
  if (existing.appType === 'codex') {
    const snapshotDir = getCodexProviderSnapshotDir(id)
    try {
      fs.rmSync(snapshotDir, { recursive: true, force: true })
    } catch {
      // Ignore cleanup failures.
    }
    removeCodexProviderSnapshot(id)
  }
...
async function restoreBackup(appType, backupId) {
  ...
  if (appType === 'codex' && latest.provider?.id) {
    writeCodexProviderSnapshot(latest.provider.id, latest.provider.config)
  }
  return latest
}
...
async function switchProvider({ appType, providerId }) {
  ...
  if (current && current.id !== target.id) {
    if (appType === 'codex') {
      writeCodexProviderSnapshot(current.id, liveBefore)
    }
  ...
  if (appType === 'codex') {
    targetConfigForSwitch = preserveProviderProfile(
      readCodexProviderSnapshot(target.id, target.config),
      target.config
    )
  }
