...
import { getMutableState, updateState } from './desktop-state'
...
async function invokeCommand<T>(command: string, args?: Record<string, unknown>): Promise<T> {
  try {
    ...
    if (cause.startsWith('AGENTS_MD_EXISTS::') || cause.startsWith('POLICY_FILE_EXISTS::')) {
...
async function refreshKitState() {
  const [kitPolicies, kitLoadouts, kits, officialPresets] = await Promise.all([
    ...
    invokeCommand<OfficialPresetSummary[]>('official_preset_list'),
  ])
  ...
  updateState((state) => {
    ...
    state.kits = kits
    state.officialPresets = officialPresets
  })
}
...
export async function refreshSkillState() {
...
export async function actionProviderList(appType?: string) {
  ...
  const providers = await invokeCommand<ProviderRecord[]>(
    'provider_list',
...
export async function actionUniversalProviderList() {
  ...
  const universalProviders =
    await invokeCommand<UniversalProviderRecord[]>('universal_provider_list')
  updateState((state) => {
    state.universalProviders = universalProviders
  })
...
export async function actionKitLoadoutImportFromRepo(values: {
  url: string
  name?: string
  description?: string
  overwrite?: boolean
  ...
  const result = await invokeCommand<KitLoadoutImportResult>('kit_loadout_import_from_repo', {
    url: values.url,
    name: values.name,
    description: values.description,
...
export async function actionKitApply(values: {
  ...
  const result = await invokeCommand<KitApplyResult>('kit_apply', {
    ...
    excludeSkills: values.excludeSkills,
  })
  await Promise.all([refreshKitState(), refreshSkillState()])
  return result
}

export async function actionOfficialPresetList() {
  if (!isTauriRuntime()) {
    return getMutableState().officialPresets
  }

  const presets = await invokeCommand<OfficialPresetSummary[]>('official_preset_list')
  updateState((state) => {
    state.officialPresets = presets
  })
  return presets
}

export async function actionOfficialPresetInspect(id: string) {
  if (!isTauriRuntime()) {
    const detail = getMutableState().officialPresetDetails[id]
    if (!detail) {
      throw new Error(`Official preset not found: ${id}`)
    }
    return detail
  }

  return invokeCommand<OfficialPresetDetail>('official_preset_get', { id })
}

export async function actionOfficialPresetInstall(values: { id: string; overwrite?: boolean }) {
  if (!isTauriRuntime()) {
    throw new Error('Tauri runtime is required.')
  }

  const result = await invokeCommand<OfficialPresetInstallResult>('official_preset_install', {
    id: values.id,
    overwrite: values.overwrite,
  })
  await Promise.all([refreshKitState(), refreshSkillState()])
...
export async function actionOfficialPresetInstallAll(values?: { overwrite?: boolean }) {
  if (!isTauriRuntime()) {
    throw new Error('Tauri runtime is required.')
  }

  const result = await invokeCommand<OfficialPresetBatchInstallResult>(
...
export async function actionKitRestoreManagedBaseline(id: string) {
  ...
  const result = await invokeCommand<KitRecord>('kit_restore_managed_baseline', { id })
  await Promise.all([refreshKitState(), refreshSkillState()])
  return result
}
...
export async function actionReorderProjects(projectPaths: string[]) {
  if (!isTauriRuntime()) {
    throw new Error('Tauri runtime is required.')
  }

  await invokeCommand<string[]>('project_reorder', { projectPaths })
...
export async function actionReorderAgents(agentNames: string[]) {
  if (!isTauriRuntime()) {
    throw new Error('Tauri runtime is required.')
  }

  await invokeCommand<string[]>('agent_reorder', { agentNames })
  await refreshConfigState()
}
